...
<effects>
<!--cut--> K(knife = ?x) ^ K(paper = ?y) => add(Kf, state(paper) = cut);
        </effects>
</action>
...
add(Kf, is_free(hand));
    ...
    <goal>
<!--goal-->K(state(paper)= cut)^ K(is_free(scissors))^ K(is_free(key))^ K(is_free(pliers))^ K(is_free(glassbox))^ K(is_free(knife))
    </goal>
<updates>
